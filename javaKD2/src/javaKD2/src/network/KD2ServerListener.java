...
public class KD2ServerListener extends Listener{
    ...
    public void connected(Connection c){
        try {
            ...
            System.out.println("Server-> Recieved a connection from " + c.getRemoteAddressTCP().getHostString());
            //create a message packet
            Packet packetMessage = new Packet();
            
            //assign the message text
            packetMessage.message = "Server-> Hello! You connected: "+new Date().toString();
            
            KD2Server server = new KD2Server();
            server.getAndSendStudents(c);//nebuus
